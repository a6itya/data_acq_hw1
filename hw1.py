...
def retrieve_data_from_gcs(bucket_name: str,
    ...
    """
    client = storage.Client.create_anonymous_client() #anonymous client to gcp bucket
    bucket = client.bucket(bucket_name) #finds bucket (bucket_name)
    blob = bucket.blob(file_name) #finds file (blob(file_name))
    contents = blob.download_as_test() #downloads blob
    return json.loads(contents) #converts from string to dict
...
if __name__ == '__main__':
    ...
    st.dataframe(filtered_df, # create this data frame by conditions.
                 hide_index=True,
                 column_config={
